...
for(i in 1:(dim(indexmatrix)[2] - 1)){
	for(j in i:dim(indexmatrix)[2]){
		plot(cors[i,j,], pch = 15, col = i, ylim = c(ymin, ymax), xaxt = "n", xlab = "", ylab = "", cex.lab = 1.5, cex.axis = 2, cex = 2)
		lines(cors[i,j,], lty = 1, col = i, lwd = 7)
		#lty = 2,1
		par(new = T)
		plot(cors[i,j,], pch = 18, col = j, ylim = c(ymin, ymax),  xaxt = "n", xlab = "", ylab = "", cex.lab = 1.5, cex.axis = 2, cex = 2)
		lines(cors[i,j,], lty = 2, col = j, lwd = 7)
		#lty =4,2
		par(new = T)
	}
}
...
for(i in 1:(dim(indexmatrix)[2] - 1)){
  for(j in (i+1):dim(indexmatrix)[2]){
    polygon(x = c(0,0,1), y = c(count-1,count-.1,count-.1), col = i, border = i)
    polygon(x = c(0,1,1), y = c(count-1,count-1,count-.1), col = j, border = j)
